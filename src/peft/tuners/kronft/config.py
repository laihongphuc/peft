...
@dataclass
class KronFTConfig(PeftConfig):
    """
    This is the configuration class to store the configuration of a [`KronFTModel`].
    ...
    Args:
        n_frequency (`int`):
            ...
            KronFT. The following examples of settings regarding 'n_frequency' can be used as reference for users.
        ...
        n_pack (`int`):
            Number of pack for the Kronecker product.
        scaling (`float`):
    ...
    n_pack: int = field(
        default=8,
        metadata={
            "help": "Number of pack for the Kronecker product."
        },
    )
    scaling: float = field(
        default=150.0,
    ...
    def __post_init__(self):
        super().__post_init__()
        self.peft_type = PeftType.KRONFT
        self.target_modules = (
